...
Every `exec` call must keep `/workspace/repo` as its `cwd`. Inspect `/project/repo` with file tools or absolute read-only paths; never set it as an `exec` working directory.
