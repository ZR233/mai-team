...
export function SkillMentionPicker({ endpoint, selected, onChange }: { endpoint: string; selected: string[]; onChange(next: string[]): void }) {
  const query = useQuery({ queryKey: ["composer-skills", endpoint], queryFn: () => api<SkillsResponse>(endpoint), staleTime: 30_000 })
  const skillsByMention = new Map((query.data?.skills ?? []).map((skill) => [skill.name, skill]))
  ...
  return <>
    ...
    <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="sm" className="shrink-0" aria-label="Select skills"><AtSign data-icon="inline-start" /> Skills{selected.length > 0 && <Badge variant="secondary">{selected.length}</Badge>}</Button></DropdownMenuTrigger><DropdownMenuContent align="start" className="max-h-80 w-80 overflow-y-auto"><DropdownMenuLabel>Skills</DropdownMenuLabel><DropdownMenuSeparator /><DropdownMenuGroup>
