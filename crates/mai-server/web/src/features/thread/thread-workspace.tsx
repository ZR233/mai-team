...
export function ThreadWorkspace({ agent, onSend, onStop, onAgentUpdated, headerActions, skillsEndpoint = "/skills", workspaceCrumbs }: ThreadWorkspaceProps) {
  ...
  return <section className="flex h-full min-h-0 flex-1 flex-col bg-background">
    ...
    <div className="shrink-0 border-t bg-background px-3 py-3 md:px-6"><div className="mx-auto max-w-5xl"><div className="mb-2 flex flex-wrap items-center justify-between gap-2 text-xs text-muted-foreground"><ConnectionStatus status={live.connection} message={live.connectionMessage} /><div className="flex gap-4"><span>Model <strong className="font-medium text-foreground">{live.snapshot?.runtime?.usage.model || agent.model}</strong></span><span>Tokens <strong className="font-medium text-foreground">{(live.snapshot?.runtime?.usage.totalTokens ?? agent.token_usage.totalTokens).toLocaleString()}</strong></span><span className="flex items-center gap-2">Context <strong className="font-medium text-foreground">{contextLabel(live.snapshot?.runtime?.usage.latestContextTokens, live.snapshot?.runtime?.usage.contextWindow)}</strong><Progress className="w-16" value={contextPercent(live.snapshot?.runtime?.usage.latestContextTokens, live.snapshot?.runtime?.usage.contextWindow)} /></span></div></div>
