...
function ReviewDetailContent({ projectId, job, detail, loading, error, retry }: { projectId: string; job: ReviewJobSummary | null; detail?: ReviewJobDetail; loading: boolean; error: unknown; retry(): void }) {
  ...
  return <div className="flex flex-col gap-5 pb-4">
    <div className="flex flex-wrap items-center gap-2"><StatusBadge status={value.status} /><ReviewOutcome job={value} /><EnvironmentWarningBadge job={value} />{value.status === "retry_waiting" && <Badge variant="outline">Retry scheduled</Badge>}</div>
    ...
    {value.environment_warning && <Alert className="border-amber-500/50 bg-amber-500/5 text-amber-950 dark:text-amber-100"><CircleAlert /><AlertTitle>Latest image refresh failed</AlertTitle><AlertDescription><span className="block">{value.environment_warning.message}</span><span className="mt-1 block text-xs opacity-80">{value.environment_warning.image} · cached {shortSha(value.environment_warning.cached_image_id)} · {formatDate(value.environment_warning.observed_at)}</span></AlertDescription></Alert>}
...
function EnvironmentWarningBadge({ job }: { job: ReviewJobSummary }) {
