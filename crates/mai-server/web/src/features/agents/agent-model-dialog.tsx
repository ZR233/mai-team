...
export function AgentModelDialog({ agent, onSaved }: { agent: AgentDetail; onSaved(): Promise<unknown> }) {
  ...
  const selectModel = (nextModelId: string) => {
    const next = provider?.models.find((candidate) => candidate.slug === nextModelId)
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        {providers.data && <FieldGroup>
          <Field><FieldLabel>Provider</FieldLabel><Select value={providerId} onValueChange={selectProvider}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent><SelectGroup>{providers.data.providers.map((item) => <SelectItem key={item.id} value={item.id}>{item.config.name}</SelectItem>)}</SelectGroup></SelectContent></Select></Field>
...
function modelReasoningOptions(model?: ProviderModel) {
  const effort = model?.parameters?.find((parameter) => parameter.name === "effort")
  return (effort?.candidates ?? []).map((item) => ({ id: item, label: item }))
}

function defaultReasoning(model?: ProviderModel) {
  return modelReasoningOptions(model)[0]?.id || "default"
}
