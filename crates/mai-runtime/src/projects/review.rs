...
Before submitting the review, inspect the PR's changed build and test entrypoints and run the relevant high-value validation in `/workspace/repo`. When the PR adds or changes an executable app, integration test, QEMU test, or guest test, execute that exact entrypoint end to end when the prepared environment provides its required toolchain. A blocking static finding does not excuse skipping this validation. Do not substitute `cargo check`, a host-only build, or GitHub CI status for an app that is intended to run in QEMU or inside a guest. Preserve the actual command and decisive build, launch, and guest output as evidence. If the exact entrypoint cannot run, record the concrete command, failure, and missing prerequisite instead of silently omitting it.
