...
use mai_protocol::{
    AgentId, AgentResourceState, AgentSummary, ProjectCloneStatus, ProjectId,
    ProjectReviewJobSummary, ProjectReviewRunStatus, ProjectReviewStatus, ProjectStatus,
...
pub(crate) async fn start_project_review_loop_if_ready(
    ...
    if worker.is_some() {
        ...
        if auto_review_enabled
            && worker
                .as_ref()
                .is_some_and(|worker| worker.relay_selector_abort_handle.is_none())
        {
            let existing = worker.as_mut().expect("review worker checked above");
            let context = ProjectReviewTaskContext {
                ops: ops.clone(),
                project_id,
                cancellation_token: existing.cancellation_token.clone(),
            };
            existing.relay_selector_abort_handle = Some(spawn_project_review_child(
...
mod tests {
    ...
    use super::{
        ...
        ProjectReviewTaskContext, ProjectReviewWorkerOps, ReviewStateUpdate,
    ...
    impl ProjectReviewWorkerOps for FakeWorkerOps {
        ...
        async fn recover_expired_project_review_jobs(
            ...
            for job in jobs.iter_mut() {
                ...
                if expired_active_job || legacy_ambiguous_submission {
                    ...
                    if legacy_ambiguous_submission {
                        job.failure = None;
    ...
    #[tokio::test]
    async fn pool_worker_waits_for_a_persisted_queue_signal() {
